...
$root = "e:\Work\Development\Projects\Hm_MES\New-HMS"
$env:PATH = 'C:\Users\vilas\AppData\Local\Programs\DockerDesktop\resources\bin;' + $env:PATH

function SlotStats($n) {
    $green = 0; $blue = 0
    for ($i = 0; $i -lt $n; $i++) {
        $hdr = & curl.exe -sk -o NUL -D - https://127.0.0.1:8443/healthz 2>$null | Select-String 'X-HMS-Slot'
...
Start-Process "$root\hms-backend\build\Release\hms-backend.exe" `
    -ArgumentList "config/drogon_config.c.json" -WorkingDirectory "$root\hms-backend" -WindowStyle Hidden
Start-Sleep 5
...
docker rm -f hms-nginx-drill 2>$null | Out-Null
docker run -d --name hms-nginx-drill -p 8443:8443 `
...
docker exec hms-nginx-drill grep -n 'green' /etc/nginx/nginx.conf
docker exec hms-nginx-drill nginx -s reload
Start-Sleep 2
$r2 = SlotStats 100
...
Get-CimInstance Win32_Process -Filter "Name='hms-backend.exe'" |
...
docker rm -f hms-nginx-drill | Out-Null
...
$leaderBefore = docker exec hms-redis redis-cli GET ws:realtime:leader
Get-CimInstance Win32_Process -Filter "Name='hms-backend.exe'" |
...
$p1 = [regex]::Match($m1, 'hms_ws_broadcast_published_total (\d+)').Groups[1].Value
Start-Sleep 6   # 租约 PX 3000, 最迟 3s 接管
$leaderAfter = docker exec hms-redis redis-cli GET ws:realtime:leader
$m2 = (Invoke-WebRequest -Uri 'http://127.0.0.1:8088/metrics' -UseBasicParsing).Content
$p2 = [regex]::Match($m2, 'hms_ws_broadcast_published_total (\d+)').Groups[1].Value
...
Start-Process "$root\hms-backend\build\Release\hms-backend.exe" `
    -ArgumentList "config/drogon_config.b.json" -WorkingDirectory "$root\hms-backend" -WindowStyle Hidden
$okB = $false
