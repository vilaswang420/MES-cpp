...
#   -> mes-iot CmdConsumer 暂停 DevicePoller -> 设备停止上报
#   -> 重启 mes-iot -> 恢复采集 (暂停态仅内存, 不持久化)
#   -> 幂等: 同线第二张工单再次报满, 重复停采指令不报错
#   -> 拓扑隔离: cmd.stop.# 仅进 iot.cmd.collector.queue, 不与后端 iot.cmd.queue 竞争消费
#
# 前置:
#   1. just dev-up (mes-postgres / mes-redis / mes-rabbitmq 三容器就绪, 迁移已应用)
#   2. mes-backend 运行于 :8088 (MES_API_BASE 可覆盖)
#   3. mes-iot 运行且 backend_pwd 已配置 (默认容器名 mes-iot, MES_IOT_RESTART_CMD 可覆盖重启命令)
...
$simPort = if ($env:MES_SIM_PORT) { [int]$env:MES_SIM_PORT } else { 15020 }
$iotRestart = if ($env:MES_IOT_RESTART_CMD) { $env:MES_IOT_RESTART_CMD } else { "docker restart mes-iot" }
$py = if ($env:MES_PYTHON) { $env:MES_PYTHON } else { "python" }
$pass = 0; $fail = 0
...
function Raw-Count([int64]$deviceId) {
    $c = docker exec mes-postgres psql -U mes -d mes -t -A -c `
...
Assert ($hz.code -eq 200) "mes-backend healthz"
$pgOk = docker exec mes-postgres psql -U mes -d mes -t -A -c "SELECT 1;"
Assert ("$pgOk" -trim -eq "1") "mes-postgres 容器可用"
$mqOk = docker exec mes-rabbitmq rabbitmq-diagnostics -q ping
Assert ($LASTEXITCODE -eq 0) "mes-rabbitmq 容器可用"

# 拓扑隔离前置: 两条 cmd 队列都必须存在 (iot.cmd.queue=后端消费原始指令, collector=cmd.stop.#)
$queues = docker exec mes-rabbitmq rabbitmqctl list_queues -q name
Assert ($queues -match "iot\.cmd\.queue") "iot.cmd.queue 存在 (后端 StopCollectionHandler)"
Assert ($queues -match "iot\.cmd\.collector\.queue") "iot.cmd.collector.queue 存在 (mes-iot CmdConsumer)"
...
# ---- 2. 重启 mes-iot 加载新设备配置 (ConfigLoader 仅启动时拉取) ----
Step "重启 mes-iot 加载新设备"
Invoke-Expression $iotRestart | Out-Null
Assert ($LASTEXITCODE -eq 0) "重启 mes-iot ($iotRestart)"
...
$relayOk = $false
for ($i = 0; $i -lt 15; $i++) {
    $n = docker exec mes-postgres psql -U mes -d mes -t -A -c `
...
# iot.cmd.queue (后端) 与 iot.cmd.collector.queue (mes-iot) 消费者 tag/连接不同;
# cmd.stop.# 只进 collector 队列, 后端队列不应堆积 cmd.stop 消息
$consumers = docker exec mes-rabbitmq rabbitmqctl list_consumers -q queue_name ack_settings
...
Assert (-not [string]::IsNullOrWhiteSpace($colQ)) "iot.cmd.collector.queue 有 mes-iot 消费者"
$qDepths = docker exec mes-rabbitmq rabbitmqctl list_queues -q name messages_ready
...
# ---- 8. 重启恢复: 暂停态仅内存, 重启 mes-iot 恢复采集 ----
Step "重启恢复: 设备恢复采集"
Invoke-Expression $iotRestart | Out-Null
Assert ($LASTEXITCODE -eq 0) "再次重启 mes-iot"
