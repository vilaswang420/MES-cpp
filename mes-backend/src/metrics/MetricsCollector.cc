...
void collectOutboxPending() {
    ...
    db->execSqlAsync(
        ...
        [](const drogon::orm::Result& r) {
            if (!r.empty())
                Metrics::gaugeSet("mes_outbox_pending", r[0][0].as<double>());
        },
    ...
    db->execSqlAsync(
        ...
        [](const drogon::orm::Result& r) {
            if (!r.empty() && !r[0][0].isNull())
                Metrics::gaugeSet("mes_partition_days_left{table=\"iot_raw_data\"}",
                                  r[0][0].as<double>());
        },
        [](const drogon::orm::DrogonDbException& e) {
...
void collectWsSubscribers() {
    ...
    Metrics::gaugeSet("mes_ws_subscribers", static_cast<double>(total));
}
...
void mqLagLoop() {
    ...
    while (!g_stop) {
        for (const char* q : queues) {
            ...
            if (n >= 0)
                Metrics::gaugeSet(std::string("mes_mq_queue_messages{queue=\"") + q + "\"}",
