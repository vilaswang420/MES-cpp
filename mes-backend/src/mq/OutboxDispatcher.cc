...
constexpr int64_t kAdvisoryLockKey = 0x484D5301; // "MES" + 投递器编号
...
drogon::Task<> tick() {
    ...
    try {
        ...
        for (const auto& row : rows) {
            ...
            if (ok) {
                Metrics::counterInc("mes_outbox_dispatched_total");
                co_await trans->execSqlCoro(
                    "UPDATE mq_outbox SET status = 1, sent_at = NOW() WHERE id = $1", SqlArg(id));
            } else {
                Metrics::counterInc("mes_outbox_dispatch_failed_total");
                ...
                if (newStatus == 3) {
                    Metrics::counterInc("mes_outbox_dead_total");
