...
> 版本: 1.1 | 日期: 2026-08-16 | 状态: **P1+P2+P3 已完成** | 更新(2026-08-20): **P4 已完成（mes-iot 容器化部署待补，见 P4_IMPLEMENTATION_PLAN.md）**
>
> ## 实施进度（2026-08-16 更新）
>
> | 批次 | 状态 | 说明 |
> |------|------|------|
> | P1 正确性 (17 项) | ✅ 全部完成 | 12 个独立提交，详见 BUILD_PROGRESS.md |
> | P2 稳定性 (5 项) | ✅ 全部完成 | Outbox 死信 / Service 单测 / 并发压测 / 降级 E2E / 告警保留 |
> | P3 安全性 (4 项) | ✅ 4.1/4.2/4.3/4.5 完成 | 验证码 SVG / 缓存去明文 / JWT 轮换 / 审计脱敏；4.4 弱凭据并入 5.3 |
> | P4 缺失功能 | ✅ 已完成（容器化待补） | IoT 采集(5.1) / 停采链路(5.2) / 大屏图表(5.3) / 真 OEE(5.4) / 前端 3 模块(5.5) / GA 压测(5.6) / IoT 管理(5.7)，代码与集成已落地 |
> | P5 增强 | ⏳ 待实施 | PAD / 导入导出 / 监控 / CI-CD |
>
> 单测: 54/54 全绿 (Ninja Release, build-ninja/)
>
> **更新（2026-08-20）**：P4（5.1–5.7）已全部实现并提交（mes-iot Modbus 采集、大屏 ECharts、真 OEE 消费者、前端 3 模块、GA 2h 脚本、IoT 管理补齐）。两点说明：① P3 安全性在 2026-08-16 本计划撰写时标注"已完成"，但 2026-08-18 发现并修复了一处严重登录认证绕过漏洞（验证缓存键不含尝试密码，导致任意密码可登录，commit 17c7385），故"P3 已完成"以修复后状态为准；② 5.1 的 mes-iot 容器化（Dockerfile.iot / compose 服务）尚未补充，属生产部署待办。
...
>
> **实施进度（2026-08-16）**：
> - P1 正确性 17 项：✅ 全部完成（12 commits）
> - P2 稳定性 5 项：✅ 全部完成（5 commits）
> - P3 安全性 4 项：✅ 4.1/4.2/4.3/4.5 完成（4.4 弱凭据并入 5.3）
> - 单测 54/54 全绿，权限映射门禁通过
> - **下一步：P4 缺失功能实现方案设计（评审通过后实施）**
