...
HMS (Human Manufacturing System) 是面向制造业的执行系统，核心设计目标：
...
> **主数据边界说明**: HMS 不维护本地物料主数据与 BOM，物料/BOM 的权威数据由 ERP (hm-erp) 持有；HMS 通过 `erp_material_code` 及本表 `material_code` 引用物料编码，领料/投料依赖 6.3 节的 WMS 领用流程。若未来需要本地 BOM，应在本模块新增物料与 BOM 表及相应 API。
...
**连接**: `wss://hms.example.com/ws/dashboard?token=JWT_TOKEN`
...
| 接口 | 方法 | 路径 | 权限码 | 说明 |
|------|------|------|--------|------|
| ERP订单同步 | POST | `/api/v1/integration/erp/sync-orders` | integ:erp:sync | 触发HMS主动拉取ERP订单(增量时间窗) |
...
sequenceDiagram
    ...
    BE->>ERP: 1. 拉取生产订单 (REST, HMS主动, 增量时间窗)
    BE->>BE: 2. 转化为工单 + 拆解工序
...
services:
  ...
  hms-iot-collector: # IoT 数据采集 (单实例按5000设备估算, 万级设备需>=3)
    build: ./hms-iot
    environment:
      - MQ_HOST=rabbitmq
    depends_on:
...
> 本文档为 HMS 制造执行系统的完整架构设计，涵盖系统架构、数据库设计(35张表)、RESTful API(50+接口)、RBAC权限控制、业务逻辑流程及高并发策略。技术栈基于 C++ Drogon 后端 + React/Vue3 前端 + PostgreSQL + Redis + RabbitMQ。
