...
| # | 缺口 | 工作量 | 依赖 |
|---|------|--------|------|
| 1 | IoT Modbus TCP 采集实现 | 2-4 周 | 无 |
| 2 | 停采指令链路打通 (两端) | 3-5 天 | #1 |
| 3 | 大屏 ECharts 集成 | 1-2 周 | 无 |
| 4 | 前端设备/质量/集成页面 | 2-3 周 | 无 |
| 5 | 验证码改为图形渲染 | 1-2 天 | 无 |
| 6 | GA 标准 2h 压测 | 1-2 天 | #1-#4 完成 |
...
| 9 | 质量追溯链修复 (处置人 + 质检联动) | 1-2 天 | 无 |
| 10 | PAD 移动端 (PWA) | 3-4 周 | #4 |
| 11 | Alertmanager + Grafana 部署 | 2 天 | 无 |
| 12 | CI/CD Pipeline 配置 | 3-5 天 | CI 已修复 |
| 13 | OEE 自动计算消费者 | 1 周 | #1 |
| 14 | IoT 域管理补齐 (传感器 CRUD/告警状态机/设备离线) | 3-5 天 | 无 |
...
| 17 | 登录验证缓存安全修复 | 半天 | 无 |
| 18 | 自签证书私钥从仓库移除 | 半天 | 无 |
| 19 | 监控 exporter 补齐 | 2 天 | #11 |
| 20 | Service 层单测补齐 | 1-2 周 | 无 |
| 21 | 缺失索引添加 | 1 天 | 无 |
| 22 | 缺失约束添加 | 1 天 | 无 |
| 23 | 多看板配置 + WS 审计 | 1 周 | 无 |
| 24 | 日志聚合 (Loki) | 1 周 | 无 |
| 25 | 设计文档漂移修正 | 1 天 | 无 |
