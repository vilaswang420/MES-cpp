...
**剩余短板**（P4 已关闭核心功能缺口，以下为仍待办项）:
- IoT 采集：真实协议采集已实现（P4-5.1），但 mes-iot 容器化（Dockerfile.iot / compose 服务）尚未补充，目前开发联调靠本地构建 + 模拟器
- PAD 移动端完全缺失（P5 增强，操作员无法在工位扫码作业）
- 监控可视化缺 Alertmanager + Grafana（P5 增强）
- 导入导出 / 计划状态流转等运营增强未实现（P5）
