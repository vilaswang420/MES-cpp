...
# 或在宿主机 docker run 挂载 hms_pg_replica 卷执行。
set -euo pipefail

PGDATA="${PGDATA:-/var/lib/postgresql/data}"
PRIMARY="${PRIMARY_HOST:-postgres-primary}"
REPL_USER="${REPL_USER:-hms}"
REPL_PASS="${REPL_PASSWORD:?set REPL_PASSWORD (= HMS_PG_PASSWORD)}"

if [ -s "$PGDATA/PG_VERSION" ]; then
    echo "数据目录已初始化, 跳过 (如需重建请先清空卷 hms_pg_replica)"
    exit 0
fi
...
PGPASSWORD="$REPL_PASS" pg_basebackup -h "$PRIMARY" -U "$REPL_USER" -D "$PGDATA" \
    -X stream -R -C -S hms_replica_slot
